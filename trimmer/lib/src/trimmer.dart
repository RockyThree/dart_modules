...
class Trimmer {
  ...
  Future<void> loadVideo({required File videoFile}) async {
    ...
    if (videoFile.existsSync()) {
      _videoPlayer = Player();
      await _videoPlayer?.open(Media(videoFile.path), play: false);
      _controller.add(TrimmerEvent.initialized);
    }
  }

  Future<void> initStreamController({required File videoFile, required Player videoPlayer}) async {
    currentVideoFile = videoFile;
  ...
  /// crash.
  ///
  Future<String> saveTrimmedVideoCommand({
  ...
  Future<bool> videoPlaybackControl({
    ...
    if (videoPlayer!.state.playing) {
      ...
      if (videoPlayer!.state.position.inMilliseconds >= endValue.toInt()) {
        await videoPlayer!.seek(Duration(milliseconds: startValue.toInt()));
